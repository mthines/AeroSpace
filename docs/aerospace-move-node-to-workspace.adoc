...
aerospace move-node-to-workspace [-h|--help] [--focus-follows-window] [--wrap-around]
                                 [--stdin|--no-stdin]
                                 (next|prev)
aerospace move-node-to-workspace [-h|--help] [--focus-follows-window]
                                 (left|right|up|down)
...
`(left|right|up|down)` is identical to `workspace (left|right|up|down)` —
overview-only grid direction; outside the overview the command is a silent no-op.
